...
Last reconciled: **2026-08-06** (post DG-0–DG-5 deck-generation revamp and TM-0–TM-6
template-management revamp; TD-32 filed and code-fixed, live re-registration still owed;
TD-33/34/35 closed).

**2026-08-15 partial update**: TD-32 closed as moot (Phase C of
`PLAN-LLM-DECK-PLANNING.md` deletes template registration entirely rather than fixing
it — see the row itself). This was a targeted update alongside that cutover, not a full
reconciliation pass; every other row here is unreviewed since 2026-08-06.
...
| **TD-32** | **Template registration has never once succeeded.** `engines/presenton.py` called `/api/v1/ppt/**templates**/…` (plural); the engine serves `/api/v1/ppt/**template**/…` (singular). Every registration 404'd → `fallback` → stock theme. Proven on the live engine 2026-08-06 (`404` plural / `422` singular). Second defect stacked behind it: `/template/init` returns `layouts: null`, so even the corrected path needed `/template/async` instead | 🔴 | 2026-08-06 | **CLOSED 2026-08-15** — moot, not fixed | **Moot as of `PLAN-LLM-DECK-PLANNING.md`'s Phase C cutover**: `engines/presenton.py` and Presenton template registration are deleted outright, not repaired. Rendering clones a template's own designed slides (`deck/clone.py`, `deck/renderer.py`) against an LLM-produced `DesignCatalog` (`deck/catalog.py`, LD-2); there is no engine-side registration step left to succeed or fail. See `revamp/reports/PHASE-C-REPORT.md` |
| **TD-33** | Template registration ran **inline inside the upload request** (`registry/service.py:96`) | 🟠 | 2026-08-06 | **`TM-2` — closed** | `POST /templates` returns sub-second with `pending`; an Arq job (`registry/registration.py`) drives it to terminal. Test-proven, `TM-0-6-REPORT.md` |
| **TD-34** | `RegistrationStatus.fallback` conflated four causes: no PPTX uploaded, 404, engine rejected, engine unreachable | 🟠 | 2026-08-06 | **`TM-3` — closed** | `pending` / `registered` / `failed` / `no_source` are distinct (migration `0013_template_reg_states`), test-proven |
| **TD-35** | No template deletion. The engine supports `DELETE /api/v1/ppt/template/{id}`; NoteAI exposed nothing | 🟡 | 2026-08-06 | **`TM-5` — closed** | `DELETE /api/v1/templates/{id}`, refuses 409 when any version is in use, test-proven |
