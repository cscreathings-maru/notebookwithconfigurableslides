> **ARCHIVED — superseded 2026-07-28.**
>
> Preserved because `README.md` cited it as quality evidence and the live
> `NOTEAI_TEST_REPORT.md` was overwritten with a Presenton routing debugging report.
>
> **Do not read the results below as evidence of product quality.** They were produced by
> `automation/verify_noteai_revamp.py`, a regex-over-source-files checker that executes no
> application code, starts no server, and issues no request. It reported a 100% pass rate
> while `/editor` returned 404, deck downloads were unreachable from the browser, brand
> tokens never reached the renderer, and RAG retrieval leaked across projects.
>
> See `ENGINEERING-ASSESSMENT.md` §4 (M2). The script is retired in Phase 2 (T-2.6).
